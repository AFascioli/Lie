...
app.get("/status", (req, res, next) => {
  return res.status(200).json({
    message: "Servidor Node.js Lie® 24/04/2021",
  });
});
...
app.get("/sdict", async (req, res) => {
  let diccionario = mimir.dict(comentarios);

  let dicMongo = await Diccionario.find().exec();

  for (const diccionario of dicMongo) {
    await Diccionario.findByIdAndDelete(diccionario._id).exec()
  }
